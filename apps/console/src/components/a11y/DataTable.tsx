...
export function DataTable({
  ...
  return (
    <div
      className={cn(
        "overflow-auto rounded-sm border border-edge bg-panel/40",
        className,
      )}
    >
      <table className="w-full border-collapse text-[11px]">
        {captionVisible ? (
          <caption className="px-3 py-1.5 text-left text-[10px] uppercase tracking-section text-faint">
            {caption}
          </caption>
        ) : (
          ...
          <tr className="border-b border-edge">
            {columns.map((col) => (
              <th
                ...
                scope="col"
                className={cn(
                  "px-3 py-1.5 text-[10px] font-normal uppercase tracking-section text-faint",
          ...
          {rows.length === 0 ? (
            <tr>
              <td
                ...
                className="px-3 py-8 text-center text-[11px] italic text-faint"
              >
                {emptyMessage}
              </td>
            </tr>
                ...
                {columns.map((col, ci) => {
                  ...
                  return ci === 0 ? (
                    <th
                      ...
                      scope="row"
                      className={cn(
                        "px-3 py-1.5 align-top font-normal text-ink",
                        alignClass,
                      )}
                    >
                      {content}
                    ...
                    <td
                      key={col.key}
                      className={cn(
                        "px-3 py-1.5 align-top text-muted",
                        alignClass,
                      )}
                    >
                      {content}
