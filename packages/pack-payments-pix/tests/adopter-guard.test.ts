 ...
 *
 * The canonical example is `@ibatexas/llm-provider`: the LLM proposes
 * `order.confirm` with `paymentMethod=pix`, and the adopter calls
 * `createPixPendingDeferGuard` with its own state-shape readers to
 * compose the same DEFER semantics into its own bundle. This test pins
 * that contract.
 */
