...
- **Cross-key conflict gate** (`evidence-ledger.ts`) — the falsifier RUNTIME arm, distinct from same-key last-write-wins: a DIFFERENT declared falsifier key being present this turn poisons the backed key to `conflict` → UNKNOWN. New: `EvidenceLedger.resolveAgainstFalsifiers`, the `CrossKeyConflict` type, `detectCrossKeyConflicts`. No integrity-ranked auto-resolution (inv.16).
