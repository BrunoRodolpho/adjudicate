 ...
 * Others wrap PIX in a higher-level intent like `order.confirm` because
 * their LLM operates one tier up. Both styles need the same
 * DEFER-on-pending-PIX guard logic.
 *
 * The factory here is extracted so the Pack's `pixPolicyBundle` and an
 * adopter's bespoke bundle can share the same DEFER semantics without
 * duplication.
 *
 * Canonical existing-intent-kind adopter: `@ibatexas/llm-provider`'s
 * `order-policy-bundle.ts` composes `createPixPendingDeferGuard`
 * against its own `order.confirm` intent kind. See the Pack README's
...
/**
 * Default settled-status set covering common provider vocabularies.
 * Adopters whose providers use other labels supply their own set via
 * `confirmedStatuses`.
 */
