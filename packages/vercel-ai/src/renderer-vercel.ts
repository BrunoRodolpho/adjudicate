/**
 * Vercel AI SDK prompt renderer.
 *
 * Mirrors `createOpenAIPromptRenderer` but tuned for the AI SDK:
 *   - Same dotted → underscored tool-name translation. The AI SDK's tool
 *     name pattern matches OpenAI's (`^[a-zA-Z0-9_-]+$`) — dots rejected.
 *   - System-prompt copy stays close to the OpenAI version. The AI SDK is
 *     provider-agnostic; the same "MUST NOT" / "Rules" language carries.
 *     Pack metadata (id) is included for traceability.
 *
...
export interface VercelPromptRendererOptions {
...
export const DEFAULT_VERCEL_ADJUDICATED_SYSTEM_PROMPT = [
...
export function createVercelPromptRenderer<S, C = unknown>(
  options: VercelPromptRendererOptions,
): PromptRenderer<S, C> {
  ...
  return {
    render(
      ...
      // intent-kind names to AI-SDK form. The AI SDK's tool name pattern
      // matches OpenAI's — dots rejected.
      ...
      segments.push(DEFAULT_VERCEL_ADJUDICATED_SYSTEM_PROMPT);
